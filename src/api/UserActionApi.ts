...
export const NewChatRequest = async (username: string, title: string, currTitle: string): Promise<ApiResponse> => {
  ...
  try {
    const response = await makeSimplePostRequest("http://192.168.0.100:8080/new_chat", { username: username, title: title, prevtitle: currTitle });
...
export const RenameChatRequest = async (username: string, currTitle: string, newTitle: string) => {
  ...
  try {
    const response = await makeSimplePostRequest("http://192.168.0.100:8080/rename_chat", { username: username, title: newTitle });
...
export const DeleteChatRequest = async (username: string, title: string): Promise<ApiResponse> => {
  ...
  try {
    const response = await makeSimplePostRequest("http://192.168.0.100:8080/delete_chat", { username: username, title: title });
    console.log(response);
    return response.data;
